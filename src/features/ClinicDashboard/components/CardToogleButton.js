...
const StyledToggleButton = styled(StyledButton)`
  ...
  align-self: center;
  border-radius: 6px;
  background-color: ${props => props.active ? constants.formBackgroundColor : 'transparent'};
  background-image: url(${props => props.active ? iconDotBurgerActive : iconDotBurger});
...
const CardToggleButton = ({onClick}) => {
    ...
    return (
        <StyledToggleButton onClick={handler} active={isActive}/>
    )
}
